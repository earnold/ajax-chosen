(function() {
  (function($) {
    return $.fn.ajaxChosen = function(options, itemBuilder) {
      ...
      select = this;
      this.chosen();
      return this.next('.chzn-container').find(inputSelector).bind('keyup', function() {
        var field, userDefinedSuccess, val;
        val = $.trim($(this).attr('value'));
        if (val.length < defaultedOptions.minLength || val === $(this).data('prevVal')) {
          return false;
        }
        ...
        defaultedOptions.success = function(data) {
          var currentOpt, currentOptions, items, newOpt, newOptions, _fn, _fn2, _i, _j, _len, _len2;
          if (!(data != null)) {
            return;
          }
